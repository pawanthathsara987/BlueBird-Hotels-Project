...
const createVisitorBooking = async (req, res) => {
    ...
    try {
        ...
        // -----------------------------
        // 1. Validate input & Handle Reception Source
        // -----------------------------
        let finalGuestId = guestId;

        if (source === 'reception') {
            if (!guestDetails || !guestDetails.firstName || !guestDetails.lastName || !guestDetails.phoneNumber) {
                return res.status(400).json({
                    success: false,
                    message: "Guest details (firstName, lastName, phoneNumber) are required for reception bookings"
                });
            }

            const email = guestDetails.email || `guest-${guestDetails.phoneNumber.replace(/[^0-9]/g, '')}@bluebird.com`;
