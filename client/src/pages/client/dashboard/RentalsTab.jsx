...
import { Car, MapPin, X, Clock, Calendar, CreditCard, AlertTriangle, Compass, Star, Eye } from "lucide-react";
...
export default function RentalsTab({ vehicles, setVehicles, isEmptyState, filterList }) {
  ...
  const bookingVehicles = filteredVehicles.filter(v => v.rawStatus !== "pending_payment");
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 font-sans">
      ...
      {isEmptyState || bookingVehicles.length === 0 ? (
        renderEmptyState(
        ...
        <div className="space-y-6">
          {bookingVehicles.map(v => (
            ...
            >
              {/* Vehicle Image */}
              <div className="relative w-full md:w-72 h-48 md:h-auto shrink-0 bg-slate-50/50 flex items-center justify-center overflow-hidden">
                <img
                  src={v.image}
                  alt={v.model}
                  className="w-full h-full object-contain p-1 transition-transform duration-700 hover:scale-105"
                  onError={(e) => {
                    e.target.src = "https://images.unsplash.com/photo-1549399542-7e3f8b79c341?auto=format&fit=crop&w=600&q=80";
                  }}
                />
                <div className="absolute top-4 left-4 z-10">
                  <span className={`px-2.5 py-1 rounded-full text-[9px] font-black tracking-wider uppercase border backdrop-blur-md shadow-sm ${statusColor(v.rawStatus)}`}>
                    {v.status}
                  </span>
                </div>
              </div>
              ...
              <div className="p-5 md:p-6 flex-1 flex flex-col justify-between space-y-4">
                <div className="space-y-3">
                  <div className="space-y-1">
                    <span className="text-[9px] font-bold text-amber-500 tracking-wider uppercase block">{v.bookingNo}</span>
                    <h3 className="font-serif font-semibold text-lg text-blue-950 leading-snug">{v.model}</h3>
                    <p className="text-xs text-slate-500 flex items-center gap-1">
                      <Car size={11} className="text-slate-400 shrink-0" />
                      {v.type} • <span className="capitalize">{(v.hireType || "").replace("_", " ")}</span>
                    </p>
                  </div>

                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 bg-slate-50/60 p-4 rounded-2xl border border-slate-100/40 text-xs">
                    <div>
                      <span className="text-[10px] text-slate-400 font-bold block mb-1">PICKUP</span>
                      <span className="font-semibold text-blue-950 flex items-center gap-1 truncate" title={v.pickupLocation}>
                        <MapPin size={9} className="text-slate-450 shrink-0" />
                    ...
                    <div>
                      <span className="text-[10px] text-slate-400 font-bold block mb-1">DROPOFF</span>
                      <span className="font-semibold text-blue-950 flex items-center gap-1 truncate" title={v.dropoffLocation}>
                        <MapPin size={9} className="text-slate-455 shrink-0" />
                    ...
                    <div>
                      <span className="text-[10px] text-slate-400 font-bold block mb-1">DURATION</span>
                      <span className="font-semibold text-blue-950 flex items-center gap-1">
                        <Clock size={9} className="text-slate-455 shrink-0" />
                        {v.numDays} Day{v.numDays !== 1 ? "s" : ""}
                      </span>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-400 font-bold block mb-1">TOTAL COST</span>
                      <span className="font-serif font-semibold text-blue-950">{import.meta.env.VITE_CURRENCY_TYPE || "LKR"} {v.price.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap justify-end gap-2.5 pt-2 border-t border-slate-100/50">
                  ...
                  {canCancel(v.rawStatus) && (
                    <button
                      ...
                      className="px-4 py-2 bg-red-50 border border-red-200 text-red-650 font-semibold text-xs rounded-xl hover:bg-red-100 transition-colors"
                    >
                      {v.payment ? "Refund & Cancel" : "Cancel Rental"}
                    </button>
                  )}
                </div>
              </div>
      ...
      {(() => {
        ...
        return (
          <>
            ...
            />
            {/* Panel */}
            <div className="fixed top-0 right-0 h-full w-full max-w-md z-50 flex flex-col bg-white shadow-2xl overflow-hidden animate-in slide-in-from-right duration-300">
              {/* Header */}
              <div className="p-5 border-b border-slate-200 bg-blue-950 text-white flex justify-between items-start">
                <div className="space-y-1">
                  <span className="text-[9px] font-bold text-amber-400 tracking-wider uppercase">Vehicle Rental Details</span>
                  <h3 className="font-serif font-semibold text-base">{v.model}</h3>
                  <p className="text-[10px] text-slate-350">{v.bookingNo}</p>
                </div>
                <button
                  onClick={() => setSelectedVehicle(null)}
              ...
              <div className="flex-grow overflow-y-auto p-5 space-y-4">
                {/* Hero Banner Image */}
                <div className="relative h-64 w-full rounded-2xl overflow-hidden shrink-0 bg-slate-50 flex items-center justify-center shadow-sm">
                  <img
                    src={v.image}
                    alt={v.model}
                  ...
                  <div className="absolute top-3 right-3 z-10">
                    <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold tracking-wider uppercase border backdrop-blur-md shadow-sm ${statusColor(v.rawStatus)}`}>
                      {v.status}
                    </span>
                  </div>
                </div>
