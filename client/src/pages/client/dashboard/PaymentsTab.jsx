    ...
    <body>
      <div class="card">
        ...
        <table>
          ...
          <tr><td>Currency</td><td>${pay.currency || "LKR"}</td></tr>
          ${pay.notes ? `<tr><td>Notes</td><td>${pay.notes}</td></tr>` : ""}
          <tr class="total-row"><td>Amount</td><td>${pay.isRefund ? "- " : ""}${pay.currency || "LKR"} ${fmt(pay.amount)}</td></tr>
        </table>
    ...
    <div className="space-y-6 animate-in fade-in duration-300 font-sans">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          {
            label: "Total Paid",
            value: `LKR ${fmt(summary.totalPaid)}`,
          ...
          {
            label: "Refunded",
            value: `LKR ${fmt(summary.totalRefunded)}`,
          ...
          {
            label: "Pending",
            value: `LKR ${fmt(summary.totalPending)}`,
            icon: <Clock size={18} />,
                ...
                <tr key={pay.id} className="hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="py-3.5 px-5 text-right">
                    <span className={`text-sm font-bold font-serif ${pay.isRefund ? "text-purple-700" : pay.status === "Failed" ? "text-rose-600" : "text-blue-950"}`}>
                      {pay.isRefund ? "- " : ""}{pay.currency || "LKR"} {fmt(pay.amount)}
                    </span>
                  </td>
                  {/* Status */}
                  <td className="py-3.5 px-5">
