...
const BookingRoom = () => {
  ...
  useEffect(() => {
    ...
    // Load airport pickup details if present
    try {
      const pickupRaw = localStorage.getItem("airportPickUp");
      if (pickupRaw) {
        const pickupData = JSON.parse(pickupRaw);
        if (pickupData && typeof pickupData === "object") {
          setAirportPickup(Boolean(pickupData.enabled));
          setPickupTime(pickupData.time || "");
        }
      }
    } catch (err) {
      // ignore parse errors
  ...
  const handleRangeChange = (dates) => {
    ...
    if (
      normalizedStart &&
      normalizedEnd &&
      normalizedStart.toDateString == normalizedEnd.toDateString
    ) {
  ...
  const totalAdults = rooms.reduce(
    (sum, room) => sum + (room.packageId ? room.adults : 0),
    0,
  );
  const totalKids = rooms.reduce(
    (sum, room) => sum + (room.packageId ? room.kids : 0),
    0,
  );
  ...
  const saveAirportPickupToStorage = (enabled, time = "") => {
    try {
      const payload = { enabled: Boolean(enabled), time: time || "" };
      localStorage.setItem("airportPickUp", JSON.stringify(payload));
    } catch (err) {
  ...
  const handleBooking = () => {
    ...
    if (!airportPickup && localStorage.getItem("airportPickUp")) {
      ...
      <section className="relative z-10 mx-auto -mt-16 max-w-7xl px-4 sm:px-8 lg:px-14">
        <div className="overflow-visible rounded-3xl border border-white/60 bg-white/95 shadow-2xl backdrop-blur-md">
          <div className="flex flex-wrap gap-2 border-b border-stone-200 p-4 sm:p-5">
            <label className="rounded-full bg-emerald-700 px-5 py-2 text-xs font-bold uppercase tracking-widest text-white">
              Hotels
            </label>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 p-4 sm:p-5 lg:grid-cols-4 lg:gap-0">
            ...
            <div className="rounded-2xl border border-stone-200 p-4 lg:rounded-none lg:border-y-0 lg:border-l-0 lg:border-r">
              ...
              <p className="mt-2 flex items-center gap-2 text-sm font-semibold text-stone-800">
                ...
                {totalAdults} Adult{totalAdults !== 1 ? "s" : ""}, {totalKids}{" "}
                Kid
                {totalKids !== 1 ? "s" : ""}
              </p>
            </div>
          ...
          <div className="border-t border-stone-200 p-4 sm:p-5">
            ...
            {rooms.length === 0 ? (
              <p className="rounded-xl border border-dashed border-stone-300 bg-stone-50 p-3 text-sm text-stone-600">
                Add a room first. Then select a package, and only after that
                choose adults and kids.
              </p>
            ) : (
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                {rooms.map((room, index) => {
                  ...
                  const adultMax = selectedPackage
                    ? selectedPackage.maxAdults
                    : 0;
                            ...
                            <div className="rounded-xl border border-emerald-200 bg-emerald-50/40 p-3">
                              ...
                              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    togglePackagePicker(room.id, true)
                                  }
                            ...
                            <div className="mb-2 flex items-center justify-between">
                              ...
                              {room.packageId && (
                                <button
                                  type="button"
                                  onClick={() =>
                                    togglePackagePicker(room.id, false)
                                  }
                              ...
                              {packageOptions.length === 0 ? (
                                ...
                                packageOptions.map((item) => {
                                  ...
                                  const remainingAvailable =
                                    getRemainingAvailability(item.id);
                                  const isUnavailable =
                                    remainingAvailable === 0 && !isSelected;
                                  ...
                                  return (
                                    <button
                                      ...
                                      onClick={() =>
                                        updateRoomPackage(room.id, item.id)
                                      }
                                    ...
                                    >
                                      ...
                                      <div className="space-y-1 p-3">
                                        ...
                                        <p className="text-xs font-semibold text-stone-700">
                                          Capacity: {item.maxAdults} Adults /{" "}
                                          {item.maxKids} Kids
                                        </p>
                ...
                <div className="group relative flex items-center">
                  <span className="absolute bottom-full left-1/2 mb-3 w-48 -translate-x-1/2 scale-0 rounded-lg bg-gray-300 border p-3 text-xs text-black shadow-lg transition-all group-hover:scale-100">
                    <p className="leading-relaxed">
                      ...
                      if available at the time of your stay.
                    </p>
                    {/* tail arrow */}
            ...
            <div className="mt-6 rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-cyan-50 p-5 shadow-sm">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div>
                        ...
                        Enjoy a comfortable pickup service directly from the airport.
                        </p>
                    </div>
                    </div>

                    {/* Toggle */}
                    <label className="flex cursor-pointer items-center gap-3 rounded-xl bg-white px-4 py-3 shadow-sm">
                    <input
                        type="checkbox"
                        name="pickup"
                        ...
                        onChange={(e) => {
                          ...
                          saveAirportPickupToStorage(checked, pickupTime);
                        }}
                ...
                {/* TIME SELECT (only show if pickup enabled) */}
                {airportPickup && (
                    <div className="mt-4">
                    ...
                    <input
                        ...
                        onChange={(e) => {
                          ...
                          setPickupTime(t);
                          saveAirportPickupToStorage(airportPickup, t);
                        }}
                        className="mt-2 w-full rounded-lg border border-stone-300 px-3 py-2 text-sm font-semibold focus:border-emerald-600 focus:outline-none"
                    />
                    </div>
                )}
                </div>
          ...
          <div className="border-t border-stone-200 p-4 sm:p-5">
            <div className="flex justify-end">
              <button
                ...
                disabled={
                  rooms.length === 0 || rooms.some((room) => !room.packageId)
                }
      ...
      <section className="mx-auto mt-12 max-w-7xl px-4 sm:px-8 lg:px-14">
        <div className="mb-6 space-y-2 gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="max-w-xl text-sm text-stone-600">
            Check the room package details below before you assign a package to
            any room.
          </p>
        </div>
