...
export default function ManagerProfileSettings() {
    ...
    useEffect(() => {
        const fetchManagerProfile = async () => {
            ...
            try {
                ...
                const managerData = res.data.find((user) => user.userId === currentUserId);

                if (managerData) {
                    setStaffId(managerData.staffId || "");
                    setName(managerData.name || "");
                    setUserName(managerData.userName || "");
                    setEmail(managerData.email || "");
                    setPhoneNumber(managerData.phoneNumber || "");
                    setNicNumber(managerData.nicNumber || "");
                    setAddress(managerData.address || "");
                    setImageUrl(managerData.imageUrl || "");
                    setImagePreview(managerData.imageUrl || "");
                    setRoleId(managerData.roleId);
                } else {
    ...
    const handleUpdateProfile = async (e) => {
        ...
        try {
            ...
            if (updatedImageUrl) {
                localStorage.setItem("managerImageUrl", updatedImageUrl);
            }
    ...
    const getInitials = (n) =>
        n ? n.split(" ").map((w) => w[0]).join("").toUpperCase().slice(0, 2) : "MG";
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-4 flex flex-col gap-6">
                        ...
                        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 flex flex-col items-center">
                            ...
                            <h2 className="text-lg font-bold text-slate-800">{name || "Manager"}</h2>
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full pl-8 pr-4 py-3 bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-xl cursor-not-allowed opacity-75 text-sm text-slate-500 placeholder-slate-450 font-medium"
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="manager@bluebird.com"
                                                    required
                                                    className="w-full pl-9 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-400 transition-all text-sm text-slate-700 placeholder-slate-400"
                                                />
                                            </div>
